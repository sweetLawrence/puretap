...
const MonthDivider = ({ label, count }: { label: string; count: number }) => (
  <div className="flex items-center gap-3 mt-2 mb-1">
    ...
    <Text size="xs" className="text-text-200">{count} reading{count !== 1 ? 's' : ''}</Text>
  </div>
)
...
export default function Readings() {
  ...
  const GroupedReadingCards = ({ data }: { data: Reading[] }) => {
    const groups = groupByMonth(data, 'reading_date')
  ...
  const GroupedReadingsTable = ({ data }: { data: Reading[] }) => {
    const groups = groupByMonth(data, 'reading_date')
    if (data.length === 0) return (
      <div className="text-center text-text-300 py-8">No readings found</div>
    )
    return (
            ...
            {groups.map(group => (
              <>
                <Table.Tr key={group.label + '-divider'}>
                  <Table.Td colSpan={8} className="bg-gray-50 py-2 px-4">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full bg-primary-400" />
                      ...
                      <Text size="xs" className="text-text-200">
                        {group.items.length} reading{group.items.length !== 1 ? 's' : ''}
                      </Text>
                    </div>
                  </Table.Td>
                </Table.Tr>
  ...
  const TabContent = ({ data, isLoading }: { data: Reading[], isLoading: boolean }) => {
    ...
    if (isMobile) return <GroupedReadingCards data={data} />
    return (
      <Paper shadow="xs" radius="lg" className="bg-white overflow-hidden">
        <GroupedReadingsTable data={data} />
      </Paper>
    )
  }
