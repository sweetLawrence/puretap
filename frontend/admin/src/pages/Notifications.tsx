...
// --- Moved outside parent to prevent remount on each keystroke ---
...
function CustomerSearch({
  ...
  section: string
  customerSearch: Record<string, string>
  customerLoading: Record<string, boolean>
  customerResults: Record<string, Customer[]>
  onSearch: (section: string, query: string) => void
  onSelect: (section: string, customer: Customer) => void
}) {
  return (
    <div>
      ...
      <input
        ...
        onChange={e => onSearch(section, e.target.value)}
...
function InvoiceSelect({
  ...
  section: string
  selectedCustomer: Record<string, Customer | null>
  invoicesLoading: Record<string, boolean>
  invoices: Record<string, Invoice[]>
  selectedInvoice: Record<string, Invoice | null>
  onSelectInvoice: (section: string, inv: Invoice) => void
}) {
  if (!selectedCustomer[section]) return null
  return (
    <div>
      <Text size="sm" fw={500} className="text-text-500 mb-2">Select Invoice</Text>
      {invoicesLoading[section] ? (
        ...
        <Stack gap="xs">
          {(invoices[section] || []).map(inv => (
            <button
              key={inv.id}
              onClick={() => onSelectInvoice(section, inv)}
              className={`w-full text-left px-4 py-3 rounded-lg border transition-all ${
                selectedInvoice[section]?.id === inv.id
...
export default function Notifications() {
  ...
  const selectInvoice = (section: string, inv: Invoice) => {
    setSelectedInvoice(prev => ({ ...prev, [section]: inv }))
  }
