...
export default function Meters() {
  ...
  const handleToggleMeter = async (meter: any) => {
    const action = meter.is_active ? 'deactivate' : 'activate'
    if (!confirm(`${meter.is_active ? 'Deactivate' : 'Activate'} meter ${meter.serial_no}?`)) return
    try {
      if (meter.is_active) {
        await api.patch(`/meters/${meter.id}/deactivate`)
      } else {
        await api.patch(`/meters/${meter.id}/activate`)
      }
      load()
    } catch {
      alert(`Failed to ${action} meter`)
    }
  }
                      ...
                      <Table.Tr key={m.id}>
                        ...
                        <Table.Td>
                          <Group gap="xs">
                            ...
                            {m.is_active ? (
                              <Tooltip label="Deactivate meter">
                                <ActionIcon variant="light" color="red" radius="md" size="sm"
                                  onClick={() => handleToggleMeter(m)}>
                                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
