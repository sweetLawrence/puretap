...
export default function SubmitReading() {
  const [step, setStep] = useState(0)
  const [scanning, setScanning] = useState(false)
  const [loadingMeter, setLoadingMeter] = useState(false)  // ← New: For meter loading
  const [meterDetails, setMeterDetails] = useState<MeterDetails | null>(null)
  const [lastReading, setLastReading] = useState<LastReading | null>(null)
  const [manualReading, setManualReading] = useState('')
  const [photoDataUrl, setPhotoDataUrl] = useState<string | null>(null)
  ...
  const [successReading, setSuccessReading] = useState<any>(null)
  const [manualSerial, setManualSerial] = useState('')
  const [loadingManualSerial, setLoadingManualSerial] = useState(false)  // ← New: For manual serial loading
  ...
  const loadMeter = async (serialNo: string) => {
    setError('')
    setLoadingMeter(true)  // ← Show spinner
    ...
    try {
      ...
      setLoadingMeter(false)  // ← Hide spinner
    }
  }
  ...
  const handleManualLoad = async () => {
    ...
    await loadMeter(manualSerial.trim())
  }
              ...
              <div className="bg-black aspect-square">
                ...
                {uploading && (
                  <div className="absolute inset-0 bg-black/70 flex items-center justify-center z-10">
                    <div className="bg-white rounded-xl p-4 flex flex-col items-center gap-3">
                      <div className="w-8 h-8 border-4 border-primary-500 border-t-transparent rounded-full animate-spin" />
                      <Text size="sm" className="text-text-600">Uploading photo...</Text>
                    </div>
                  </div>
                )}
              </div>
