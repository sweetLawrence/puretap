...
export default function AppLayout() {
  ...
  return (
    <AppShell
      header={{ height: 56 }}
      navbar={isMobile ? undefined : { width: 220, breakpoint: 'md', collapsed: { mobile: !opened } }}
      padding={0}
    >
      {/* Header */}
      <AppShell.Header className="border-b border-gray-100 bg-white flex items-center px-4 gap-3">
        ...
        <div className="ml-auto flex items-center gap-3">
          <div className="text-right hidden sm:block">
            <Text size="xs" fw={600} className="text-text-600">{customer?.full_name}</Text>
            <Text size="xs" className="text-text-300">{customer?.account_no}</Text>
          </div>
          <UnstyledButton onClick={handleLogout}
            className="text-text-300 hover:text-red-500 transition-colors p-1">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" />
              <polyline points="16 17 21 12 16 7" />
              <line x1="21" y1="12" x2="9" y2="12" />
      ...
      {/* Main content */}
      <AppShell.Main className="bg-back-500" style={{ paddingBottom: isMobile ? '64px' : '0' }}>
        <Outlet />
      </AppShell.Main>
            ...
            {navItems.map(item => {
              ...
              return (
                <button key={item.path}
                  onClick={() => navigate(item.path)}
                  className={`flex flex-col items-center gap-0.5 px-3 py-1 transition-colors ${active ? 'text-primary-500' : 'text-text-300'
                    }`}>
                  {item.icon}
                  <span className={`text-xs leading-tight ${active ? 'font-semibold text-primary-500' : 'text-text-300'}`}>
                    {item.label}
                  </span>
                </button>
              )
